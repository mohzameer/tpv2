-- Fix: Update function to properly get emails from auth.users
-- The issue is likely with the UUID to TEXT conversion
...
BEGIN
  ...
  -- Use subquery to get email from auth.users
  RETURN QUERY
  SELECT 
    pm.id,
    pm.project_id,
...
-- Test query (replace with your project ID):
-- SELECT * FROM get_project_members_with_emails('your-project-id');
